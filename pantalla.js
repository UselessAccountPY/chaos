...
// true mientras la animación está corriendo — bloquea re-entradas
let animacionCorriendo = false;
// Guarda si la última vez había pregunta activa (para detectar preguntas nuevas)
let ultimaPreguntaActiva = false;

const ANGULOS_FLECHAS = [0, 180, 270, 90, 315, 135, 225, 45];

function anguloParaFlecha(indice) {
  return -ANGULOS_FLECHAS[indice];
}
...
async function animarSeleccionPregunta(indiceFlecha, callbackFin) {
  ...
  const flechas  = svgEl.querySelectorAll("g");

  // Resetear estado visual antes de arrancar
  ...
  // ── Tiempos (fine-tune aquí) ──────────────────────────
  const VUELTAS_TOTALES = 5;
  const DURACION_SPIN   = 1800;  // ms frenando
  const DURACION_ZOOM   = 1000;  // ms del zoom
  const DURACION_FLASH  = 350;   // ms del destello
  // ─────────────────────────────────────────────────────

  const anguloFinal = VUELTAS_TOTALES * 360 + anguloParaFlecha(indiceFlecha);

  // Pequeño delay para que el reset de transform se aplique antes de la transición
  await new Promise(r => setTimeout(r, 30));

  // FASE 1 — Spin con frenado
  wrap.style.transition = `transform ${DURACION_SPIN}ms cubic-bezier(0.25, 0.1, 0.1, 1)`;
  wrap.style.transform  = `rotate(${anguloFinal}deg)`;

  // FASE 2 — Iluminar flecha elegida cuando casi frenó
  await new Promise(r => setTimeout(r, DURACION_SPIN * 0.65));
  ...
  // FASE 3 — Zoom cuando terminó el spin
  await new Promise(r => setTimeout(r, DURACION_SPIN * 0.35));
  wrap.style.transition = `transform ${DURACION_ZOOM}ms cubic-bezier(0.4, 0, 1, 1)`;
  wrap.style.transform  = `rotate(${anguloFinal}deg) scale(22)`;

  // FASE 4 — Flash
  await new Promise(r => setTimeout(r, DURACION_ZOOM));
  flash.style.transition = `opacity ${DURACION_FLASH * 0.3}ms ease-in`;
  flash.style.opacity = "1";

  await new Promise(r => setTimeout(r, DURACION_FLASH * 0.3));
